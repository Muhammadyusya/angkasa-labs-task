...
try {
    ...
    $stmt->execute([$id]);
    $task = $stmt->fetch();
    if (!$task) die("Data tidak ditemukan.");
...
<div class="max-w-3xl mx-auto">
  <div class="mb-8 flex items-center justify-between">
    ...
    <span
      class="px-3 py-1.5 bg-slate-100 text-slate-500 font-mono font-bold text-sm rounded-lg border border-slate-200">#AL-<?= sprintf('%04d', $task['id']) ?></span>
  </div>
  ...
  <div
    ...
    <form action="proses_edit.php" method="POST" class="space-y-6">
      ...
      <div class="space-y-2">
        <label class="block text-xs font-bold text-slate-700">Spesifikasi Pekerjaan <span
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="space-y-2">
          <label class="block text-xs font-bold text-slate-700">Status Dinamis</label>
          <select name="status"
            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all shadow-sm cursor-pointer">
            <option value="Pending" <?= ($task['status'] == 'Pending') ? 'selected' : '' ?>>Antrean / Pending</option>
            <option value="Dalam Proses" <?= ($task['status'] == 'Dalam Proses') ? 'selected' : '' ?>>Dalam Proses
              Eksekusi</option>
            <option value="Selesai" <?= ($task['status'] == 'Selesai') ? 'selected' : '' ?>>Selesai</option>
          </select>
        </div>
        <div class="space-y-2">
          <label class="block text-xs font-bold text-slate-700">Tenggat Waktu</label>
